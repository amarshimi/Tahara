package com.tahara.amarshimi.date;
 ...
 * along with the corresponding hebrew date. This class can use the standard
 ...
 * a class that implements a hebrew calendar version of the Calendar class, one
 * is available from <a
 ...
 * Software-- Practice & Experience, vol. 20, no. 9 (September, 1990), pp. 899-
 * 928.
 *
 * Available at <a
 * href="http://emr.cs.uiuc.edu/~reingold/calendar.ps">http://emr
 * .cs.uiuc.edu/~reingold/calendar.ps</a>. Original C++ source: <a
 * href="http://emr.cs.uiuc.edu/~reingold/calendar.C"
 * >http://emr.cs.uiuc.edu/~reingold/calendar.C</a>
 *
 * @see java.util.Date
...
public class HebrewDate implements Comparable, Cloneable {
    ...
    public String HebrowCorectdate;
    ...
    /**
     * returns the number of days are in a month.
     */
    // ND+ER
    ...
    // ND+ER //
    // Computes the absolute date from the Gregorian date. Default is current
    // date.//
    ...
    private int getHebrewCalendarElapsedDays(int year) {
        ...
        if ((conjunctionParts >= 19440) // If new moon is at or after midday,//
                ...
                // later...//
                && (isHebrewLeapYear(year - 1)))) { // at the end of a leap
            // year//
    ...
    private int getDaysInHebrewYear() {
        return ((getHebrewCalendarElapsedDays(hebrewYear + 1)) - (getHebrewCalendarElapsedDays(hebrewYear)));
    }
    ...
    public int getLastDayOfHebrewMonth(int month) {
        if ((month == 2) || (month == 4) || (month == 6)
                ...
                || ((month == 9) && isKislevShort()) || (month == 10)
                || ((month == 12) && !(isHebrewLeapYear())) || (month == 13)) {
            return 29;
        } else {
            return 30;
        }
    ...
    private void absDateToHebrewDate() {
        hebrewYear = (absDate + HEBREW_EPOCH) / 366; // Approximation from
        // below.//
    ...
    // ND+ER //
    // Computes the absolute date of Hebrew date. Default is current hebrew
    // date. //
     ...
     */
    public HebrewDate(int month, int date, int year) {
    ...
    /**
     * Sets the date based on a java.util.Calendar object. Modifies the Hebrew
     * date as well.
     */
    ...
    public void setDate(int month, int date, int year) {
        ...
        if (month > 12 || month < 0) {
            throw new IllegalArgumentException("Illegal value for month= "
                    + month);
        }
        if (date < 0) {
            throw new IllegalArgumentException("Illegal value for date= "
                    + date);
        }
        ...
        if (year < 0) {
            throw new IllegalArgumentException("Illegal value for year= "
                    + year);
        }
     ...
     */
    public void setHebrewDate(int month, int date, int year) {
        if (month < 1 || month > getLastMonthOfHebrewYear()) {
            throw new IllegalArgumentException(
                    "Illegal value for the hebrew month= " + month);
        }
        if (date < 0) {
            throw new IllegalArgumentException(
                    "Illegal value for hebrew date= " + date);
        }
        // make sure date is valid for this month, otherwise, set to last day of
        // month //
        ...
        if (year < 0) {
            throw new IllegalArgumentException(
                    "Illegal value for the hebrew year= " + year);
        }
     ...
     * "Month day, year" e.g. "Teves 23, 5760"
     */
    public int getHebrewDayAsInt() {

        return hebrewDate;

    }

    public String getHebrewDayAsString() {
        switch (hebrewDate) {
            case 1:
                HebrowCorectdate = "א";
                break;
    ...
    public String getHebrewDateAsString() {
        ...
        return HebrowCorectdate + " " + getHebrewMonthAsString();
        //return getHebrewMonthAsString() + " " + hebrewDate + ", " + hebrewYear;
    }

    /**
     * Returns a string containing the Gregorian date in the form,
     * "Month day, year" e.g. "January 1, 2000".
     * <p/>
     * For more "standard" date processing, use getTime and then the
    ...
    /**
     * Rolls the date forward by 1. It modifies both the gregorian and hebrew
     * dates accordingly
     */
    public void forward() {
        ...
        if (hebrewDate == getLastDayOfHebrewMonth()) {
            ...
            if (hebrewMonth == 6) {
                ...
                hebrewMonth++;
                hebrewDate = 1;
            } else if (hebrewMonth == getLastMonthOfHebrewYear()) { // if it is
                // the last
                // day of
                // Adar, or
                // Adar II
    ...
    public void back() {
        ...
        if (hebrewDate == 1) {
            ...
            } else if (hebrewMonth == 7) { // if rosh hashana //
                hebrewYear--;
                hebrewMonth--;
            } else {
                hebrewMonth--;
     ...
     */
    public void setHebrewMonth(int month) {
     ...
     */
    public void setHebrewYear(int year) {
        setDate(CURRENT_MONTH, CURRENT_DATE, year);
    }
     ...
     */
    public void setHebrewDate(int date) {
